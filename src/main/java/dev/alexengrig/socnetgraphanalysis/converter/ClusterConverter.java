...
@Component
public class ClusterConverter implements Converter<Map<Centroid, List<Record>>, Parent> {

    @Override
    public Parent convert(Map<Centroid, List<Record>> source) {
        List<Node> target = new ArrayList<>(source.size());
        int i = 1;
        for (Map.Entry<Centroid, List<Record>> entry : source.entrySet()) {
